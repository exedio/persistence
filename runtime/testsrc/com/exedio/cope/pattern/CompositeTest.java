...
public class CompositeTest extends CopeAssert
{
	...
	{
		final Value value = new Value("overrideDefault");
	...
	public void testOverrideDefaultNull()
	{
		...
		assertEquals("1234", value.getString4());
		assertEquals(null,   value.getStringDefault());
		assertEquals(4,      value.getIntMax4());
		assertEquals(5l,     value.getLongField());
		assertEquals(6.6,    value.getDoubleField());
		assertEquals(false,  value.getBooleanField());
	...
	static final class Value extends Composite
	{
		...
		public static final StringField stringDefault = new StringField().optional().defaultTo("defString");
		...
		Value(final String stringDefault)
		{
			this(new com.exedio.cope.SetValue[]{
					Value.string4.map("1234"),
					Value.stringDefault.map(stringDefault),
					Value.intMax4.map(4),

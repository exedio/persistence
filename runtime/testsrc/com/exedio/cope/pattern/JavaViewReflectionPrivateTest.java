...
public class JavaViewReflectionPrivateTest
{
	@Test void testType()
	{
		assertEquals(String.class, MyItem.view.getValueType());
		assertEquals(String.class, MyItem.view.getValueGenericType());
	}

	@Test void testGet()
	{
		final MyItem item = activate(MyItem.TYPE, 55);
		assertEquals("getter result", item.getView());
		assertEquals("getter result", MyItem.view.get(item));
	}
	...
	static
	{
		MODEL.enableSerialization(JavaViewReflectionPrivateTest.class, "MODEL");
	}
}

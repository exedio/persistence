	...
	{
		{
			...
			final Join join = query.join(PointerTargetItem.TYPE, PointerItem.code.is(PointerTargetItem.code));
		...
		}
		{
			final Query<PointerItem> query = PointerItem.TYPE.newQuery(null);
			assertEqualsUnmodifiable(list(), query.getJoins());
			final Join join = query.join(PointerTargetItem.TYPE, PointerItem.code.toUpperCase().is(PointerTargetItem.code.toUpperCase()));
			assertEqualsUnmodifiable(list(join), query.getJoins());
			assertContains(leftJoined, query.search());
		}
		{
			final Query<PointerItem> query = PointerItem.TYPE.newQuery(null);
			assertEqualsUnmodifiable(list(), query.getJoins());
			final Join join = query.join(PointerTargetItem.TYPE, PointerItem.code.toUpperCase().is(PointerTargetItem.code));
			assertEqualsUnmodifiable(list(join), query.getJoins());
		...
		}
		{
			final Query<PointerItem> query = PointerItem.TYPE.newQuery(null);
			assertEqualsUnmodifiable(list(), query.getJoins());
			final Join join = query.joinOuterLeft(PointerTargetItem.TYPE, PointerItem.code.is(PointerTargetItem.code));
			assertEqualsUnmodifiable(list(join), query.getJoins());
		...
		}
		{
			final Query<PointerItem> query = PointerItem.TYPE.newQuery(null);
			assertEqualsUnmodifiable(list(), query.getJoins());
			final Join join = query.joinOuterRight(PointerTargetItem.TYPE, PointerItem.code.is(PointerTargetItem.code));
			assertEqualsUnmodifiable(list(join), query.getJoins());

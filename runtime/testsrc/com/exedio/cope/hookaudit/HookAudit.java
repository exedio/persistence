...
final class HookAudit extends Item
{
	static final StringField value = new StringField().toFinal();

	static List<String> search()
	{
		final Query<String> query = new Query<>(value);
		query.setOrderBy(TYPE.getThis(), true);
		return query.search();
	}


	/**
	 * Creates a new HookAudit with all the fields initially needed.

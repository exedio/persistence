...
public class PlusIntegerTest extends AbstractRuntimeTest
{
	...
	public void testSum()
	{
		...
		assertEquals(i6, item.getMultiply23());
		...
		assertContains(item, item.TYPE.search(item.multiply23.equal(6)));
		assertContains(item, item.TYPE.search(item.num1.plus(item.num2).equal(3)));
		assertContains(item, item.TYPE.search(item.num2.multiply(item.num3).equal(6)));
		...
		assertContains(item, item.TYPE.search(item.num1.multiply(item.num2).equal((Integer)null)));
	}
}

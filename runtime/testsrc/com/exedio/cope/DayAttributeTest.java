...
public class DayAttributeTest extends AbstractLibTest
{
	...
	public void testIt()
	{
		...
		try
		{
			item.set(item.day, Integer.valueOf(10));
			fail();
		}
		catch(ClassCastException e)
		{
			assertEquals("expected " + Day.class.getName() + ", got " + Integer.class.getName() + " for day", e.getMessage());
		}
	}
	
	protected static Collection search(final DayAttribute selectAttribute)
	{
		return search(selectAttribute, null);
	}
	
	protected static Collection search(final DayAttribute selectAttribute, final Condition condition)
	{

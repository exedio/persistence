...
public class IntegerTypeTest extends TestWithEnvironment
{
	...
	@Test public void testItemCreation()
	{
		...
		}
		{
			...
			checkDataType(
					...
					"INTEGER", "int", "int", "NUMBER(10)", "integer");
		}
		{
			...
			checkDataType(
					...
					"INTEGER", "mediumint", "int", "NUMBER(7)", "integer");
		}
		{
			...
			checkDataType(
					...
					"SMALLINT", "smallint", "int", "NUMBER(5)", "smallint");
		}
		{
			...
			checkDataType(
					...
					"TINYINT", "tinyint", "int", "NUMBER(3)", "smallint");
		}
	}
	...
	private void setPkTo(final Type<?> type, final long lastUsedPkValue) throws SQLException
	{
		...
		if (!model.getConnectProperties().primaryKeyGenerator.persistent)
		{
			...
			final String pkColumnName = getPrimaryKeyColumnName(type);
			final String update = "UPDATE " + quoteName(MODEL, tableName) + " SET " + quoteName(MODEL, pkColumnName) + " = ? ";
	...
	protected void checkDataType(
			...
			final String expectedOracleDataType,

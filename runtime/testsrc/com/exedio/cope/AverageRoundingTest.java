...
public class AverageRoundingTest extends TestWithEnvironment
{
	...
	{
		assertEquals(expectedInt,    new Query<>(intx   .average()).searchSingleton(), 0.001,     "int");
		assertEquals(expectedLong,   new Query<>(longx  .average()).searchSingleton(), 0.001,     "long");
		assertEquals(expectedDouble, new Query<>(doublex.average()).searchSingleton(), 0.0000005, "double");
	}
}

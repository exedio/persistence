...
final class OracleDialect extends Dialect
{
	...
	OracleDialect(final CopeProbe probe)
	{
		...
		probe.environmentInfo.requireDatabaseVersionAtLeast("Oracle", 11, 2);
	}
	...
	{
		return "SELECT COUNT(*) FROM DUAL WHERE EXISTS (";
	}
	...
	QueryInfo explainExecutionPlan(final Statement statement, final Connection connection, final Executor executor)
	{
		...
		final String statementID = STATEMENT_ID_PREFIX + (statementIDNumber!=Integer.MIN_VALUE ? Math.abs(statementIDNumber) : Integer.MAX_VALUE);

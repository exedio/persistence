	...
	node('GitCloneExedio && docker')
	{
		try
		{
			abortable
			{
				...
				updateGitlabCommitStatus name: 'Main', state: 'running'
		...
		finally
		{
			...
			updateGitlabCommitStatus name: 'Main', state: currentBuild.resultIsBetterOrEqualTo("SUCCESS") ? "success" : "failed" // https://docs.gitlab.com/ee/api/commits.html#post-the-build-status-to-a-commit
